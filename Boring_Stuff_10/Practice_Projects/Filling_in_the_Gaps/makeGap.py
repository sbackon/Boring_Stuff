...
for j in range(len(numbers), gap-1, -1):
    shutil.copy(prefix + str(integer_list[j-1]).zfill(3) + '.txt', prefix + str(j+1).zfill(3) +'.txt')

os.remove(prefix + str(integer_list[gap-1]).zfill(3) + '.txt')
